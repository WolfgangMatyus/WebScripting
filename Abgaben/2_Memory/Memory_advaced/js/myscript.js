...
function aktivieren(item){
    if(item.id.includes("cold")){
        ...
        item.style.backgroundImage = "url(../Memory/pics/memoryBg.png)";
    }   
}
...
function dreheKarte(){
    if((Number(pick1)+Number(pick2)) == 17){
        ...
        if(count == 8){
            ...
            printScore(spieler, min, sec, totalTrys);
        }
    }else{
...
function printScore(spieler, min, sec, totalTrys){
        score = calcScore(min, sec, totalTrys);
        scoreList.push(String(score+","+spieler+","+min+","+sec+","+totalTrys));       
        scoreList.sort();
    const scorebereich = document.getElementById("score");
    while (scorebereich.hasChildNodes()) {scorebereich.removeChild(scorebereich.firstChild);}
    for(s=0; s<scoreList.length;s++){    
        printline = scoreList[s].split(",");
        var line = document.createElement("li");
        line.setAttribute("id", printline[0]);
        document.getElementById("score").appendChild(line);
    }
    for(s=0; s<scoreList.length;s++){
        document.getElementById(printline[0]).innerHTML = printline[1]+"\nZeit: "+printline[2]+":"+printline[3]+" Versuche: " + printline[4] +"\n";
    }
}

function calcScore(min, sec, totalTrys){
    return Number(totalTrys*10000)+Number(min*100+sec);
}
